<template>
  <div class="app-container">
    ...
    <el-dialog :visible.sync="userDialog" :title="user.username">
      <div>真实姓名: {{user.name}} </div>
      <div>固定电话: {{user.phone}} </div>
      <div>手机: {{user.mobile}} </div>
      <div>Email: {{user.email}} </div>
      <div>QQ: {{user.qq}} </div>
      <div>邮政编码: {{user.postcode}} </div>
      <div>详细地址: {{user.address}} </div>
      <div>接鸽地点: {{user.receive_address}} </div>
      <div>接鸽人: {{user.receiver}} </div>
      <div>血统书接受地址: {{user.lineage_address}} </div>
      <div>血统书收件人: {{user.lineage_receiver}} </div>
      <div>保证金: {{user.balance}} </div>
    </el-dialog>
  </div>
</template>
